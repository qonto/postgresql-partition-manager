...
// CutoverCmd returns the "convert cutover" sub-command.
func CutoverCmd() *cobra.Command {
	var dryRun bool

	cmd := &cobra.Command{
		Use:   "cutover [table-name]",
		Short: "Atomically swap source and target tables",
		Long:  "Perform the atomic cutover: acquire locks, drain CDC queue, rename tables",
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if err != nil {
				if errors.Is(err, convert.ErrLockTimeout) {
					log.Error("Lock timeout during cutover", "error", err)
					os.Exit(LockTimeoutExitCode)
				}

				if errors.Is(err, convert.ErrQueueNotEmpty) {
					log.Error("CDC queue assertion failed during cutover", "error", err)
					os.Exit(AssertionFailedExitCode)
				}

				log.Error("Cutover failed", "error", err)
				os.Exit(CutoverFailedExitCode)
			}

			log.Info("Cutover completed successfully",
