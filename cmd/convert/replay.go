...
const (
	ReplayFailedExitCode = 8
)

// ReplayCmd returns the "convert replay" sub-command.
func ReplayCmd() *cobra.Command {
	var dryRun bool

	cmd := &cobra.Command{
		Use:   "replay [table-name]",
		Short: "Replay CDC events from the queue onto the target table",
		Long:  "Apply captured change events (INSERT, UPDATE, DELETE) from the CDC queue to the target partitioned table",
